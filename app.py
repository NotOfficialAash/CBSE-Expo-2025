...
def switch_signal_with_yellow(prev, next):
    set_signal_state(prev, red=False, green=False)
    set_yellow(prev, True)
    time.sleep(2)
    set_yellow(prev, False)
    set_signal_state(prev, red=True, green=False)

    set_signal_state(next, red=False, green=False)
    set_yellow(next, True)
    time.sleep(2)
    set_yellow(next, False)
    set_signal_state(next, red=False, green=True)
